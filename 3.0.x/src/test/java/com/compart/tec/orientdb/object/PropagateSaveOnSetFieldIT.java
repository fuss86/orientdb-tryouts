...
public class PropagateSaveOnSetFieldIT extends AbstractOrientDBObjectITest {
    ...
    public void testPropagateSaveOverSet_SameConnection() {
        ...
        peter.setFavoriteFood("raclette");
        ...
        retrievedPeter.setFavoriteFood("scargots");
        this.getDatabase().save(retrievedAyrton);

        // verify
        retrievedAyrton = this.getDatabase().load(new ORecordId(ayrtonId));
        Assert.assertEquals("scargots", retrievedAyrton.getFriends().iterator().next().getFriends().iterator().next().getFavoriteFood());
    }
